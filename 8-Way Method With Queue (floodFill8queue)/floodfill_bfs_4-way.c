...
#include <math.h>
#include "gfx.h"
// cc floodfill_bfs_4-way.c gfx.c -o floodfill_bfs_4-way -I/usr/X11R6/include -L/usr/X11R6/lib -lX11 -lm
...
void bfs_floodfill_4way(int fill_color, int wall_color, int n, int m, int r, int a[][m][r])
{
    ...
    Zeroing(&queue_x);
    Zeroing(&queue_y);
    int Delta_x[4] = {0, A, 0, -A};                    //
    int Delta_y[4] = {-A, 0, A, 0};                    //
    ...
    {
        for (i = 0; i < 4; i++) // обследуем 4 направления
        {
...
int main()
{
    ...
    bfs_floodfill_4way(fill_color, wall_color, n, m, r, (int(*)[m][r])a);
