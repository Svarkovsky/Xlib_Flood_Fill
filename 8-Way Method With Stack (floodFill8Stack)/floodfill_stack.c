...
#include <math.h>
#include "gfx.h"
// cc floodfill_stack.c gfx.c -o floodfill_stack -I/usr/X11R6/include -L/usr/X11R6/lib -lX11 -lm
...
void InitStack(Stack *apArr)
{
    apArr->Top = 0;
}
int Push(Stack *apArr, int Value)
{ // Stack *apArr > &stack > stack
    if (apArr->Top >= ARR_SIZE)
    {
        printf("Стек полон.\n");
        return -1;
    }
    apArr->Array[apArr->Top] = Value; // Вставить элемент.
    apArr->Top++;                     // Передвинуть указатель.
}
int Pop(Stack *apArr)
{
    apArr->Top--;
    ...
    return apArr->Array[apArr->Top];
}
void Print(Stack *apArr) // функция вывода массива
{
    for (int i = 0; i < apArr->Top; i++)
    {
        printf("[ %d ]", apArr->Array[i]);
    }
    printf("\n");
}
int isEmpty(Stack *apArr)
{
    if (apArr->Top <= 0)
        return -1;
}
int Zeroing(Stack *apArr) // зануление массива
{
...
void dfs_floodfill_8way(int fill_color, int wall_color, int n, int m, int r, int a[][m][r])
{                  //  Depth-first search
    Stack stack_x; // объявление переменных структур
    Stack stack_y;
    InitStack(&stack_x); // инициализация структур
    InitStack(&stack_y);
    Zeroing(&stack_x);
    Zeroing(&stack_y);
    Print(&stack_x);
    ...
    do // цикл с постусловием
    {
        ...
        m_filling(x, y, A); // параметры краски должны быть указаны перед вызовом функции в main()
        x = Pop(&stack_x);  // присвоили и удалили из стека
        y = Pop(&stack_y);
        //    Print(&stack_x);
        //  Print(&stack_y);
    } while (-1 != isEmpty(&stack_x) && -1 != isEmpty(&stack_y));
}
...
int main()
{
    ...
    dfs_floodfill_8way(fill_color, wall_color, n, m, r, (int(*)[m][r])a);
